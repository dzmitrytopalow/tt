...
def get_types_publishing():
    ...
    markup.row('Закреплённый пост')
    markup.row('Пост вне очереди')
...
@bot.message_handler(func=lambda message: message.text == 'Бесплатная публикация'
                     or message.text == 'Закреплённый пост'
                     or message.text == 'Пост вне очереди')
def check_username(message: types.Message):
    ...
    if username is None:
        ...
        markup.row('Отмена')
        markup.resize_keyboard = True
        if message.text == 'Бесплатная публикация':
        ...
        elif message.text == 'Закреплённый пост':
            ...
            db_access.set_user_state(message.from_user.id, states.WRITE_TEXT_FOR_POST)
        elif message.text == 'Пост вне очереди':
            text = 'Пост вне очереди\n\n💡(После редактирования поста, с тобой свяжется один из ' \
...
def add_photo(message: types.Message):
    ...
    if message.content_type == 'photo':
        ...
        queue = post.queue
        text = 'Создан отложенный пост в канал «toptovar» \n\n*Твое место в общей очереди на ' \
...
def queue_of_post(message: types.Message):
    ...
    text = 'Cводка по очереди на публикацию ✅ \n\n' \
...
@bot.message_handler(func=lambda message: message.text == '✅ Следующая публикация 👉'
                     and (message.from_user.id == ADMIN_DIMA_ID))
...
if 'HEROKU' in list(os.environ.keys()):
    ...
    def webhook():
        bot.remove_webhook()
        bot.set_webhook(url='https://toptovar.herokuapp.com/' + TOKEN)
        return '!', 200
